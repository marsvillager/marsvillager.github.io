...
- 有限域 $GF(p)$ 中，$p$ 为素数，其加法和乘法需要对结果进行 $\bmod p$，以保证结果都是域中的元素
...
- 因此引入 $GF(p^n)$，$p$ 为素数，通常取 $2$
  ...
  - 加法 $f(x) + g(x) = x^3 + (2 \bmod 2)x^2 + x + 2 \bmod 2 = x^3 + x$
...
- 有限域 $\mathbb{F}_p$ 上的椭圆曲线群
  ...
  - > ⚠️ 与传统的有限域上的离散对数不同，使用点加法而非乘法，或者说其对乘法的定义为 $a·P = P + P + \cdots + P$
  ...
  - 有限域 $\mathbb{F}_p$ 上的椭圆曲线群 $E=\{(x,y) \in (\mathbb{F_p})^2 | y^2 = x^3 + ax + b \ (\bmod p),4a^3+27b^2 \neq 0 \ (\bmod p) \} \bigcup \{ O \}$（$O$ 表示无穷远点）

  - 设 $P_1, P_2$ 为椭圆曲线上两个关于 $x$ 轴对称的点即 $P_1 = (x, y), P_2 = (x, -y)$，$P_1P_2$ 连线的延长线为无穷远，故 $P_1, P_2, O$ 三点共线，$P_1 + P_2 + O = O$，故 $P_1 = -P_2$，即称 $P_1, P_2$ 互为加法逆元，即负元，e.g. $(x, y)$ 的负元是 $(x, -y \bmod \ p) = (x, p-y)$
